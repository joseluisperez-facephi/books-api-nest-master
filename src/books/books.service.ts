...
export class BooksService {
  ...
  async findOne(term: string) {
    ...
    if (isUUID(term)) {
      ...
      const queryBuilder = await this.bookRepository
        .createQueryBuilder()
        .where('UPPER(titulo) = :inputTitulo', {
          inputTitulo: term.toUpperCase(),
        })
        .orWhere('autor = :inputAutor', {
          inputAutor: term.toLowerCase(),
        })
        .getOne();
    }
    ...
    if (!Book)
      throw new NotFoundException(
        `El producto con el id ${term} no ha sido encontrado`,
      );
  ...
  async update(id: string, updateBookDto: UpdateBookDto) {
    ...
    if (!Book)
      throw new NotFoundException(
        `El libro con el id: ${id} no ha sido encontrado`,
      );
  ...
  async remove(id: string) {
    ...
    if (Book.id === Book.id) await this.bookRepository.remove(Book);
  }
